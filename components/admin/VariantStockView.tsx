...
export default function VariantStockView({ locationId: initialLocation }: { locationId: 'DEPOT' | 'BOUTIQUE' }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const r = await fetch(`/api/admin/variant-stock?${new URLSearchParams({ ...filters, locationId, groupBy: 'product', page: String(page) })}`, { signal: c.signal, cache: 'no-store' });
  ...
  function handleAdjust(r: Row) {
    const mode = r.trackingMode ?? 'SIMPLE';
    if (mode === 'SIMPLE' || !r.migrationRequired === false) {
      // SIMPLE products: open compact modal
      // Also open simple modal for BOUTIQUE regardless (BOUTIQUE always starts SIMPLE)
      if (locationId === 'BOUTIQUE' && mode === 'SIMPLE') {
        setSimpleAdjust(r);
      } else if (locationId === 'DEPOT' && mode === 'SIMPLE') {
        setSimpleAdjust(r);
      } else {
        setConfiguring(r);
      }
  ...
  // Simpler version: SIMPLE = compact modal, VARIANT = full matrix
  function openAdjust(r: Row) {
    const mode = r.trackingMode ?? (locationId === 'BOUTIQUE' ? 'SIMPLE' : 'SIMPLE');
    if (mode === 'SIMPLE') {
      setSimpleAdjust(r);
    } else {
      setConfiguring(r);
  ...
  return (
    <div className="p-4 sm:p-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500">
            ...
            {locationId === 'DEPOT' && ' Stock exact par taille et couleur ou global selon le mode.'}
          </p>
        </div>
      ...
      <div className="mb-5 flex flex-wrap gap-3 rounded-2xl border bg-white p-4">
        ...
        {(['productId', 'status', 'sort'] as const).map(key => (
          <select aria-label={key} className="input w-auto" key={key} value={filters[key]} onChange={e => filter(key, e.target.value)}>
            {(key === 'productId'
              ? [['', 'Tous les produits'], ...data.products.map(p => [p.id, p.name])]
              : key === 'status'
              ? [['', 'Tous les stocks'], ['in', 'En stock'], ['low', 'Stock faible'], ['out', 'Épuisé']]
              : [['name', 'Produit A-Z'], ['available', 'Disponible croissant']]
      ...
      <div className="overflow-x-auto rounded-2xl border bg-white">
        <table className="w-full whitespace-nowrap text-left text-sm">
          ...
          <tbody>
            {data.items.map(r => (
              <tr className="border-t" key={r.productId}>
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    ...
                    {(r.trackingMode === 'VARIANT' || r.migrationRequired) && (
                      <button className="btn-ghost text-xs" onClick={() => setConfiguring(r)}>Détails</button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      {simpleAdjust && (
        <SimpleStockModal
          ...
          productName={simpleAdjust.productName}
