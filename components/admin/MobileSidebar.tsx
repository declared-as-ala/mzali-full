...
export default function MobileSidebar({ role, open, onClose }: { role?: string; open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen w-72 flex-col border-r border-slate-800/80 bg-slate-900 p-4 text-slate-300 select-none overflow-y-auto custom-scrollbar transition-transform duration-300 ease-out lg:hidden ${
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition"
